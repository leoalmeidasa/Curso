...
class AccountCurrent extends Account
{
    ...
    {
        $this->balance += $value;
        Trigger::show("Déposito de  {$this->toBrl($value)} efetuado com sucesso!", Trigger::ACCEPT);
    }
    ...
    final public function withdrawal($value)
    {
        if($value <= $this->balance + $this->limit) {
            ...
            Trigger::show("Saldo insuficiente, você tem {$this->toBrl($this->limit)}", Trigger::ERROR);
        }
    }
